% Compact UC3M + ISCIII template, for documents with very little room
% (one-page summaries, short hand-ins, etc.). English version of main.tex:
% same package, same layout; the only change is the babel line below.
% Demo of the minimal title block with the logo row, compact sections
% (with a "run-in" subsubsection), the \reference line, a list, a table, a
% figure and a bibtex bibliography (its "References" heading comes from
% babel-english). One-column version; see main-twocolumn.tex (Spanish)
% for the same package in two-column mode.
%
...
% babel goes BEFORE uc3misciitight: the package loads hyperref internally,
...
engine~\cite{megiddo2003}, explicitly invalidated on every write to the
affected tables~\cite{silberschatz2019}.
